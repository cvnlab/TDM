...
% surface mesh (TR) represented in one of the following formats:
%
%  (1) 'triangulation' object, 
%  (2) 'TriRep' object, 
%  (3) 1-by-2 cell such that TR={Tri,V}, where Tri is a M-by-3 or M-by-4
%      array of faces and V is a N-by-3 array of vertex coordinates, or
...
%            triangular mesh, and M-by-4 array when TR is a quad mesh.
...
if ~isnumeric(Tri) || ~ismatrix(Tri) || c<3 || c>4 || ~isequal(Tri,round(Tri)) || sum(Tri(:)<0)>0
    error('Face-vertex connectivity list must be specified as a M-by-(3 or 4) array of POSITIVE integers')
end

d=size(X,2);
if ~isnumeric(X) || ~ismatrix(X) || sum(isinf(X(:)))>0 || d<2 || d>3 
